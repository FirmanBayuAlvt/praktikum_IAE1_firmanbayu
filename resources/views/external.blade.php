...
<html>
<head>
    <title>Dashboard Saham {{ $meta['2. Symbol'] ?? '' }}</title>
...
<body>
    <div class="hero text-center">
        <h1 class="display-5 fw-bold mb-2"><i class="fa-solid fa-chart-line"></i> Dashboard Saham {{ $meta['2. Symbol'] ?? '' }}</h1>
        <p class="lead mb-0">Data real-time, harian, dan adjusted dari Alpha Vantage API</p>
        <small>Last Refreshed: {{ $meta['3. Last Refreshed'] ?? '-' }} | Interval: {{ $meta['4. Interval'] ?? '-' }}</small>
    </div>
    <div class="container">
        <div class="row mb-4">
            <div class="col-md-12">
                <div class="d-flex flex-wrap gap-3 justify-content-center mb-4">
                    <div class="card card-summary shadow-sm" style="min-width:180px;">
                        <div class="card-body text-center">
                            ...
                            <h3 class="fw-bold mb-0 text-primary">{{ $last ? number_format($last['close'], 2) : '-' }}</h3>
                        </div>
                    </div>
                    <div class="card card-summary shadow-sm" style="min-width:180px;">
                        <div class="card-body text-center">
                            ...
                            <h3 class="fw-bold mb-0"
                                style="color:{{ $change > 0 ? '#198754' : ($change < 0 ? '#dc3545' : '#6c757d') }};">
                                {{ $change > 0 ? '+' : '' }}{{ number_format($change, 2) }}
                            </h3>
                            <span class="small"
                                style="color:{{ $changePct > 0 ? '#198754' : ($changePct < 0 ? '#dc3545' : '#6c757d') }};">
                    ...
                    <div class="card card-summary shadow-sm" style="min-width:180px;">
                        <div class="card-body text-center">
                            ...
                            <h3 class="fw-bold mb-0 text-success">{{ $last ? number_format($last['volume']) : '-' }}</h3>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-10">
                <div class="card mb-4">
                    <div class="card-header bg-primary text-white">
                        ...
                        <small>{{ $meta['3. Last Refreshed'] ?? '' }} | Interval: {{ $meta['4. Interval'] ?? '' }}</small>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    @forelse($chartData as $row)
                                        @php $isUp = $row['close'] >= $row['open']; @endphp
                                        <tr style="background:{{ $isUp ? '#e9fbe9' : '#fbe9e9' }};">
                                            <td>{{ $row['time'] ?? '-' }}</td>
                                            <td>{{ isset($row['open']) ? number_format($row['open'], 2) : '-' }}</td>
                                            <td>{{ isset($row['high']) ? number_format($row['high'], 2) : '-' }}</td>
                                            <td>{{ isset($row['low']) ? number_format($row['low'], 2) : '-' }}</td>
                                            <td style="color:{{ $isUp ? '#198754' : '#dc3545' }}; font-weight:bold;">
                                                {{ isset($row['close']) ? number_format($row['close'], 2) : '-' }}</td>
                                            <td>{{ isset($row['volume']) ? number_format($row['volume']) : '-' }}</td>
                                        </tr>
                                    @empty
                                        <tr>
        ...
        <div class="col-md-10">
            <div class="card mb-4">
                <div class="card-header bg-success text-white">
                    ...
                    <small>{{ $dailyMeta['3. Last Refreshed'] ?? '' }}</small>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered">
        ...
        <div class="col-md-10">
            <div class="card mb-4">
                <div class="card-header bg-warning text-dark">
                    ...
                    <small>{{ $adjustedMeta['3. Last Refreshed'] ?? '' }}</small>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                                @forelse($adjustedChartData ?? [] as $row)
                                    @php $isUp = $row['close'] >= $row['open']; @endphp
                                    <tr style="background:{{ $isUp ? '#fffbe9' : '#fbe9e9' }};">
                                        <td>{{ $row['date'] ?? '-' }}</td>
                                        <td>{{ isset($row['open']) ? number_format($row['open'], 2) : '-' }}</td>
                                        <td>{{ isset($row['high']) ? number_format($row['high'], 2) : '-' }}</td>
                                        <td>{{ isset($row['low']) ? number_format($row['low'], 2) : '-' }}</td>
                                        <td style="color:{{ $isUp ? '#198754' : '#dc3545' }}; font-weight:bold;">
                                            {{ isset($row['close']) ? number_format($row['close'], 2) : '-' }}</td>
                                        <td>{{ isset($row['adjusted_close']) ? number_format($row['adjusted_close'], 2) : '-' }}</td>
                                        <td>{{ isset($row['volume']) ? number_format($row['volume']) : '-' }}</td>
                                        <td>{{ isset($row['dividend']) ? number_format($row['dividend'], 4) : '-' }}</td>
                                        <td>{{ isset($row['split']) ? number_format($row['split'], 2) : '-' }}</td>
                                    </tr>
                                @empty
                                    <tr>
